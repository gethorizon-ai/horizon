"""Test API methods related to prompt object."""
...
def test_list_prompts(test_client):
    """Test API method to list prompts."""
    with test_client.application.app_context():
        ...
        response = test_client.get("/api/prompts", headers={"X-Api-Key": u.api_key})
...
def test_create_prompt(test_client):
    """Test API method to create prompt."""
    with test_client.application.app_context():
        ...
        new_prompt = {"name": "New Prompt", "task_id": 1}
...
def test_update_prompt(test_client):
    """Test API method to update prompt."""
    with test_client.application.app_context():
        # Create a sample user and prompt
        u = User(username="john", email="john@example.com", password="cat")
        db.session.add(u)
        ...
        update_data = {"version": "v2.0", "status": "completed", "source": "new_source"}
...
def test_delete_prompt(test_client):
    """Test API method to delete prompt."""
    with test_client.application.app_context():
        # Create a sample user and prompt
        u = User(username="john", email="john@example.com", password="cat")
        db.session.add(u)
