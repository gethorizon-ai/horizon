...
class InferenceEvaluationResults(pd.DataFrame):
    ...
    def __init__(
        ...
        prompt-model candidate and evaluation datum provided.

        Args:
            prompt_model_id_list (List[int]): list of unique prompt-model ids
            evaluation_data_id_list (List[int]): list of unique evaluation data ids
            stage_id (str): stage id for this inference and evaluation run
        """
        ...
        # output, inference quality, inference cost, and inference latency initialized with NaN placeholder values
        stage_id = [stage_id] * len(prompt_model_id_list)
        output_list = [np.nan] * len(prompt_model_id_list)
        inference_quality_score_list = [np.nan] * len(prompt_model_id_list)
        ...
        super().__init__(
            {
                ...
                "evaluation_data_id_": evaluation_data_id_list,
                "stage_id": stage_id,
                "output": output_list,
                "inference_quality_score": inference_quality_score_list,
